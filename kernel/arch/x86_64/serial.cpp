...
bool init_serial() {
    io::outb(registerOf(Port::COM1, Register::InterruptEnable), InterruptEnable::None); // Disable all interrupts
    io::outb(registerOf(Port::COM1, Register::LineControl), LineControl::Dlab); // Enable DLAB (set baud rate divisor)
    io::outb(registerOf(Port::COM1, Register::DataBuffer), BaudDivisorLow); // Set divisor to 1 (lo byte) 115200 baud
    io::outb(registerOf(Port::COM1, Register::InterruptEnable), BaudDivisorHigh); //                  (hi byte)
    io::outb(
        registerOf(Port::COM1, Register::LineControl), LineControl::DataBits8
    ...
    io::outb(registerOf(Port::COM1, Register::DataBuffer), LoopbackTestByte); // Test serial chip

    // Check if serial is faulty (i.e: not same byte as sent)
    if (io::inb(registerOf(Port::COM1, Register::DataBuffer)) != LoopbackTestByte) {
        return false;
    }
...
bool serialReceived() { return io::inb(registerOf(Port::COM1, Register::LineStatus)) & LineStatus::DataReady; }

uint8_t readSerial() {
    while (!serialReceived())
        ;

    return io::inb(registerOf(Port::COM1, Register::DataBuffer));
}

bool isTransmitBufferEmpty() {
    return io::inb(registerOf(Port::COM1, Register::LineStatus)) & LineStatus::TransmitEmpty;
}
...
void writeSerial(const uint8_t byte) {
    ...
    io::outb(registerOf(Port::COM1, Register::DataBuffer), byte);
}

bool tryRead(uint8_t& byte) {
    if (serialReceived()) {
        byte = io::inb(registerOf(Port::COM1, Register::DataBuffer));
        return true;
    }
    return false;
}
